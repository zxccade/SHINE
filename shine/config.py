...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--dset_name", type=str, choices=['charades', 'anet'])
        ...
        # Data config
        parser.add_argument("--max_q_l", type=int, default=32, help="charades:32, anet=50")
        parser.add_argument("--max_v_l", type=int, default=-1)
        parser.add_argument("--clip_length", type=int, default=0.5, help="charades:0.5, anet=1")
        parser.add_argument("--max_windows", type=int, default=5)

        parser.add_argument("--train_path", type=str, default=None)
        parser.add_argument("--eval_path", type=str, default=None, nargs="*",
        ...
        # Loss
        parser.add_argument("--lw_saliency", type=float, default=1.0,
        ...
        # * SHINE config
        parser.add_argument("--coarse_coef", default=1.0, type=float, help="coarse ranking loss coef")
        parser.add_argument("--fine_coef", default=1.0, type=float, help="fine ranking loss coef")
        parser.add_argument("--q", default=8, type=int, help="hyperparameter q for top-k selection in coarse ranking")
        parser.add_argument("--ranking_type", default='red', type=str, choices=['red', 'abd']) # 'red': relative distance; 'abd': absolute distance
        parser.add_argument("--hn_num", default=3, type=int, help="numbers of hard negatives")
        parser.add_argument("--coarse_margin", default=[1.0, 2.0], type=float, nargs=2)
        parser.add_argument("--fine_margin", default=[0.25, 0.25, 0.25, 0.25], type=float, nargs='+')
        parser.add_argument("--use_saliency_loss", action="store_true",
                            help="Disable saliency_loss due to the intra loss in coarse ranking")      
        self.parser = parser
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            save_fns = ['shine/model.py', 'shine/transformer.py']
