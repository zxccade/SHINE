...
from shine.span_utils import generalized_temporal_iou, span_cxw_to_xx

from shine.matcher import build_matcher
from shine.transformer import build_transformer
from shine.position_encoding import build_position_encoding
from shine.misc import accuracy
from shine.ctf_ranking import coarse_ranking, fine_ranking
...
class SHINE(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
                 ...
                 max_v_l=75, span_loss_type="l1", use_txt_pos=False, n_input_proj=2, aud_dim=0, hn_num=3):
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_hn_txt=None, src_hn_txt_mask=None, src_aud=None, src_aud_mask=None):
        ...
        if (self.training) and (src_hn_txt is not None):
            
            src_hn = []
            hn_mask = []
            pos_hn = []
            for i in range(self.hn_num):
                hn_txt = self.input_txt_proj(src_hn_txt[i])
                hn_txt_ = torch.cat([src_vid, hn_txt], dim=1)
                src_hn.append(torch.cat([src_, hn_txt_], dim=1))
                hn_mask_ = torch.cat([src_vid_mask, src_hn_txt_mask[i]], dim=1).bool()
                hn_mask.append(torch.cat([mask_, hn_mask_], dim=1))
                pos_hn.append(pos.clone())
            src_hn = torch.cat(src_hn, dim=0)
            hn_mask = torch.cat(hn_mask, dim=0)
            pos_hn = torch.cat(pos_hn, dim=0)

            _, _, memory_hn, memory_global_hn = self.transformer(src_hn, ~hn_mask, self.query_embed.weight, pos_hn, video_length=video_length)
            vid_mem_hn = memory_hn[:, :src_vid.shape[1]]

            out["saliency_scores_hn"] = (torch.sum(self.saliency_proj1(vid_mem_hn) * self.saliency_proj2(memory_global_hn).unsqueeze(1), dim=-1) / np.sqrt(self.hidden_dim))
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, eos_coef, losses, temperature, span_loss_type, max_v_l,
        ...
        self.use_saliency_loss=use_saliency_loss 
    ...
    def loss_saliency(self, outputs, targets, indices, log=True):
        ...
        # coarse-to-fine loss for composition

        loss_coarse = coarse_ranking(targets, outputs, margin1=self.coarse_margin[0], margin2=self.coarse_margin[1], q=self.q)

        loss_saliency += (loss_coarse * self.coarse_coef)
       
        if self.training:
            saliency_score_hn = outputs["saliency_scores_hn"].clone()
            for i in range(self.hn_num):
                outputs["saliency_scores_hn{}".format(i+1)]=saliency_score_hn[i*bs:(i+1)*bs]

            loss_fine = fine_ranking(targets, outputs, self.hn_num, self.fine_margin, rt=self.ranking_type)
            loss_saliency += (loss_fine * self.fine_coef)
