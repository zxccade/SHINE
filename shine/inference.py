...
from shine.config import TestOptions
...
def start_inference(train_opt=None, split=None, splitfile=None):
    ...
    for eval_path in opt.eval_path:
        print("####### Start to eval {} #######".format(eval_path))
        split_name = eval_path.split('.')[0].split('_')[-1]
        if opt.a_feat_dir is None:
        ...
        save_submission_filename = "inference_{}_{}_{}_preds.jsonl".format(
            opt.dset_name, opt.eval_split_name, split_name)
