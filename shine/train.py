...
# import wandb
# wandb.init(project="qddetr-charades-CG")
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        if opt.a_feat_dir is None:
            model_inputs, targets = prepare_batch_inputs_train(batch[1], opt.device, non_blocking=opt.pin_memory)
        else:
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_datasets, opt):
    ...
    opt.eval_log_txt_formatter = "{time_str} [Epoch] {epoch:03d} [Split] {split_name} [Loss] {loss_str} [Metrics] {eval_metrics_str}\n"
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            total_miou = 0.
            latest_file_list = []
            log_dict = {}  # This dictionary will hold metrics for all datasets
            with torch.no_grad():
                for split_name, val_dataset in val_datasets.items():
                    save_submission_filename = "latest_{}_{}_{}_preds.jsonl".format(opt.dset_name, opt.eval_split_name, split_name)
                    metrics_no_nms, metrics_nms, eval_loss_meters, latest_file_paths = \
                        eval_epoch(model, val_dataset, opt, save_submission_filename, epoch_i, criterion, tb_writer)
                    latest_file_list.append(latest_file_paths)

                    # Adding metrics for the specific subset to log_dict
                    for metric, value in metrics_no_nms["brief"].items():
                        log_key = f"{metric}/{split_name}"  # e.g., "R1@0.5_split_name"
                        log_dict[log_key] = value

                    for loss, value in eval_loss_meters.items():
                        log_key = f"eval_{loss}/{split_name}"  # e.g., "loss_span_split_name"
                        log_dict[log_key] = value.avg
                    # log
                    ...
                    with open(opt.eval_log_filepath, "a") as f:
                        f.write(to_write)
                    logger.info("{} metrics_no_nms {}".format(split_name, pprint.pformat(metrics_no_nms["brief"], indent=4)))
                    if metrics_nms is not None:
                        logger.info("{} metrics_nms {}".format(split_name, pprint.pformat(metrics_nms["brief"], indent=4)))

                    metrics = metrics_no_nms
                    total_miou += metrics["brief"]["MR-full-R1@0.5"]
                    total_miou += metrics["brief"]["MR-full-R1@0.7"]
                    total_miou += metrics["brief"]["MR-full-mIoU"]
                    for k, v in metrics["brief"].items():
                        tb_writer.add_scalar(f"Eval/{split_name}{k}", float(v), epoch_i+1)

            stop_score = total_miou
            ...
            if stop_score > prev_best_score:
                ...
                for latest_file_path in latest_file_list:
                    best_file_paths = [e.replace("latest", "best") for e in latest_file_path]
                    for src, tgt in zip(latest_file_path, best_file_paths):
                        os.renames(src, tgt)
...
def start_training():
    ...
    if opt.eval_path is not None:
        for split in opt.eval_path:
            split_name = split.split('.')[0].split('_')[-1]
            dataset_config["data_path"] = split
            dataset_config["txt_drop_ratio"] = 0
            dataset_config["q_feat_dir"] = opt.t_feat_dir.replace("sub_features", "text_features")  # for pretraining
            # dataset_config["load_labels"] = False  # uncomment to calculate eval loss
            if opt.a_feat_dir is None:
                eval_dataset = StartEndDataset(**dataset_config)
                eval_datasets[split_name] = eval_dataset
            else:
